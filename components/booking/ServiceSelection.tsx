...
export default function ServiceSelection() {
    const { selectedService, setSelectedService, apartmentSize, extras, nextStep } =
        useBookingStore();

    // ✅ FIX: локально типизируем apartmentSize под то, что ждёт calculateHours
    const sizeId = apartmentSize as ApartmentSizeId | null;
        ...
        <section className="w-full">
            ...
            <div className="mt-6 grid gap-4">
                {SERVICES.map((s) => {
                    ...
                    // ✅ hours для карточки: считаем только если size выбран
                    const hoursForCard = sizeId ? calculateHours(s.id, sizeId, extras) : null;
                ...
                <Check className="h-4 w-4" />
                            ...
                            <div className="pr-12">
                                ...
                                <ul
                                    className={
                                        s.isDark
                                            ? "mt-2 text-xs text-white/70 space-y-1"
                                            : "mt-2 text-xs text-black/60 space-y-1"
                                    }
                                >
                                    {s.includes.slice(0, 7).map((it) => (
                                        <li key={it} className="flex gap-2">
                      <span className={s.isDark ? "text-white/40" : "text-black/35"}>
                        •
                      </span>
                                            <span>{it}</span>
                                        </li>
                            ...
                            >
                                <div
                                    className={
                                        s.isDark
                                            ...
                                            : "text-xl font-semibold tracking-tight text-black"
                                    }
                                >
                                ...
                                >
                                    inc. VAT{" "}
                                    <span className={s.isDark ? "text-white/50" : "text-black/45"}>
                    • ~{hoursForCard != null ? hoursForCard : "—"}h
                  </span>
                                </div>
                            </div>
                        </button>
