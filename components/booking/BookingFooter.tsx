...
export default function BookingFooter({onSubmit, isSubmitting}: Props) {
    ...
    // ✅ Continue/BookNow: light stays solid, dark becomes outline (no white fill)
    const primaryBtn =
        ...
        "dark:bg-transparent dark:text-white dark:border-2 dark:border-white/35 " +
        "dark:hover:bg-white/5 dark:disabled:bg-transparent dark:disabled:text-white/35 dark:disabled:border-white/15";
        ...
        >
            <div className="px-4 md:px-6 pt-3">
                <div className="max-w-4xl mx-auto">
                    {/* ✅ Summary strip: убрать белый в dark, оставить наш тёмный стиль */}
                    {serviceId && (
                        <div className="mb-3">
                            <div
                                className="rounded-2xl bg-gray-900 text-white px-4 py-3 shadow-sm dark:bg-black dark:text-white border border-white/10">
                                <div className="flex items-start justify-between gap-3">
                                    <div className="min-w-0">
                                        <div className="font-semibold text-sm truncate">{serviceTitle}</div>
                                        <div className="text-xs text-white/70 truncate dark:text-white/70">
                                            {summarySubtitle || t("summary.selectDetails")}
                                        </div>
                                        <div
                                            className="text-xs text-white/50 mt-1 dark:text-white/50">{summaryMeta}</div>
                                    </div>
                                    ...
                                    <div className="text-right shrink-0">
                                        ...
                                        <div className="text-xs text-white/70 whitespace-nowrap dark:text-white/70">
                                            {showPrice
                        ...
                        <div className="flex flex-col items-end gap-1 shrink-0">
                            <div className="flex gap-2 md:gap-3 shrink-0">
                                {step > 0 && (
                                    <button
                                        onClick={back}
                                        className="px-5 md:px-8 py-3 border border-gray-300 text-gray-700 font-medium rounded-full hover:bg-gray-50 transition-all dark:bg-transparent dark:text-white dark:border-white/20 dark:hover:bg-white/5"
                                    >
                                        {t("back")}
                                    </button>
                                )}
